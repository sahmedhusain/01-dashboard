...
`,F={dashboard:{defaultTab:"dashboard",tabOrder:["dashboard","piscines","leaderboard","groups","audits","checkpoints","events","subjects"]}},Be=(n,E)=>{const A=new TextEncoder,d=A.encode(n),c=A.encode(E),x=new Uint8Array(d.length);for(let l=0;l<d.length;l++)x[l]=c[l%c.length];const b=new Uint8Array(d.length);for(let l=0;l<d.length;l++)b[l]=d[l]^x[l];let v="";for(let l=0;l<b.length;l++)v+=String.fromCharCode(b[l]);return btoa(v)},Je=(n,E,A,d)=>{const c=Be(n,E),x=new Date().toISOString();return JSON.stringify({_encrypted:!0,_format:A,_dataType:d,_timestamp:x,_instructions:{message:"This file is password protected. Use the decryption tool with your credentials.",decryption:"To decrypt: 1) Use your credential 2) Apply XOR decryption with base64 encoding 3) Parse the resulting content"},_encryptedData:c},null,2)},et=({userId:n,onClose:E,defaultTabs:A=[]})=>{const[d,c]=w.useState(F),[x,b]=w.useState(!1),[v,l]=w.useState("appearance"),[O,S]=w.useState({}),[h,X]=w.useState("users"),[m,R]=w.useState("json"),[C,P]=w.useState(!0),{user:T}=we(),{theme:D,setTheme:M,toggleTheme:W}=ve();w.useEffect(()=>{const t=localStorage.getItem(`user-preferences-${n}`);if(t)try{const s=JSON.parse(t);c({...F,...s})}catch{}},[n]);const{data:j}=De(ze,{errorPolicy:"all",fetchPolicy:"cache-first"}),[H]=_(Ie,{errorPolicy:"all"}),[k]=_(Ue,{errorPolicy:"all"}),[o]=_(Ge,{errorPolicy:"all"}),[L]=_(Fe,{errorPolicy:"all"}),[I]=_(qe,{errorPolicy:"all"}),[Z]=_(Xe,{errorPolicy:"all"}),[xe]=_(Me,{errorPolicy:"all"}),[he]=_(He,{errorPolicy:"all"}),z=(t,s,r)=>{c(a=>({...a,[t]:{...a[t],[s]:r}})),b(!0)},me=()=>{localStorage.setItem(`user-preferences-${n}`,JSON.stringify(d)),b(!1)},pe=()=>{c(F),b(!0)},Y=(t,s)=>{const r=[...d.dashboard.tabOrder],[a]=r.splice(t,1);r.splice(s,0,a),z("dashboard","tabOrder",r)},ue=()=>{z("dashboard","tabOrder",F.dashboard.tabOrder)},U=(t,s)=>{S(r=>({...r,[t]:s}))},ee=(t,s,r)=>{if(!t||t.length===0)return"";const a=C?{metadata:{exportedAt:new Date().toISOString(),dataType:s,format:r,recordCount:t.length,exportedBy:n},data:t}:t;switch(r){case"json":return JSON.stringify(a,null,2);case"csv":{if(t.length===0)return"";const i=Object.keys(t[0]).join(","),p=t.map(f=>Object.values(f).map(u=>typeof u=="string"&&u.includes(",")?`"${u.replace(/"/g,'""')}"`:u).join(","));return C?`# Exported on ${new Date().toISOString()}
# Data type: ${s}
# Records: ${t.length}
${i}
${p.join(`
