...
import { ErrorUtils, ERROR_SEVERITY, ERROR_TYPES } from '../../utils/errorHandling/errorManager';
...
const ErrorFeedback: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleError = (error: ErrorInput) => {
   ...
   */
  const handleRecoveryAction = (action: string, errorId: string): void => {
    switch (action) {
      case 'retry':
        // Trigger retry logic
   ...
   */
  const reportError = (errorId: string): void => {
   ...
   * Show notification
   */
  const showNotification = (notification: Partial<Notification>): void => {
   ...
   */
  const getErrorTitle = (error: { type?: string }): string => {
    const titles = {
   ...
   */
  const getNotificationDuration = (severity: string): number => {
    const durations = {
   ...
   */
  const getErrorIcon = (severity: string): React.ReactElement => {
    const icons = {
   ...
   */
  const getErrorColors = (severity: string): ErrorColors => {
    const colors = {
      [ERROR_SEVERITY.LOW]: {
        bg: 'bg-blue-500/10',
