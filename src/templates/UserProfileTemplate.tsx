...
import { useUserInfo, useTotalXP, useUserLevel } from '../hooks/useGraphQLData';
import {
  processUserProfile,
  processXPData,
  processLevelData,
...
const UserProfileTemplate = ({ userLogin, showStats = true, showRank = true }: { userLogin: string; showStats?: boolean; showRank?: boolean }) => {
  // Fetch user data
  const {
    data: userInfo,
    loading: userLoading,
    error: userError,
    refetch: refetchUser
  } = useUserInfo(userLogin);

  const {
    data: xpData,
    loading: xpLoading,
    error: xpError
  } = useTotalXP(userLogin);
